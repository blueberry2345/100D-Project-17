...
if list_status == "OK":
    ...
    # For each email, get the status and contents of the email matching the ID
    for id in email_id_list:
        email_status, message = imap.fetch(id, "(RFC822)")
        # If email fetch is successful, continue. Else, print error message.
        if email_status == "OK":
            # store sender, subject and content of email.
